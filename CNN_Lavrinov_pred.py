...
cnn = CNN()
cnn.load_state_dict(torch.load(FOLDER_DATASET + 'cnn+ann.ckpt'))

# формируем предсказания
image = load_image(FOLDER_DATASET + "00001.jpg")
predictions = cnn(image)
prediction = int(predictions.argmax())
print("Предсказание: ", _NAMES[prediction])

#Валидация
correct = 0
f = open('./archive/valid1.txt')
for line in f:
    path='./archive/' + line.split()[1]
    image = load_image(path)
    predictions = cnn(image)
    prediction = predictions.argmax()
    print("Факт / Предсказание: ", int(line.split()[0]), " / ", int(prediction))
    if int(prediction) == int(line.split()[0]) : correct +=1
print('\nТочность на наборе валидации {} %'.format(100 * correct / 588))